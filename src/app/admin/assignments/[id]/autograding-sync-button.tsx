...
import type { SincronizarAutogradingResult } from "@/lib/services/sincronizarAutograding";

// Encabezado de la tabla de entregas: dispara la sincronización del
// resultado de autograding contra GitHub. Al montar, sincroniza sin forzar
// (respeta la ventana de frescura del caché, así abrir la vista no martilla
// la API si ya se consultó hace poco); el botón fuerza el lote completo.
export function AutogradingSyncButton({ assignmentId }: { assignmentId: string }) {
  ...
  async function sincronizar(forzar: boolean) {
    const resultado = await call(async () => {
      const response = await fetch(`/api/assignments/${assignmentId}/autograding`, {
        method: "POST",
      ...
      return (await response.json()) as SincronizarAutogradingResult;
    });
